...
class APIClient:
    ...
    def __init__(self, config: Dict):
        ...
        self.logger.info(f"API client initialized for endpoint: {self.config['endpoint']}")
    ...
    def send_data(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Successfully sent data to API. Status: {response.status_code}")
    ...
    def _prepare_payload(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if data.get('success'):
            ...
            if 'extracted_files' in scraped_content and scraped_content['extracted_files']:
    ...
    def send_batch_data(self, data_list: list[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            if method == 'POST':
                ...
                self.logger.error(f"Batch operations only support POST method, got: {method}")
                return False
            
            response.raise_for_status()
            
            self.logger.info(f"Successfully sent batch data to API. Items: {len(data_list)}, Status: {response.status_code}")
            return True
