...
class LLMClient:
    ...
    def __init__(self, config: Dict):
        ...
        if not self.api_key:
            self.logger.warning("No LLM API key provided. LLM features will be disabled.")
    ...
    def _query_openai(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are a helpful assistant that analyzes robots.txt files. Always respond with valid JSON."
                },
                {
                    "role": "user",
                    "content": prompt
    ...
    def _query_anthropic(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": f"You are a helpful assistant that analyzes robots.txt files. Always respond with valid JSON.\n\n{prompt}"
                }
            ]
        }
    ...
    def _query_generic(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error("Could not extract content from LLM response")
            return None
    ...
    def _extract_json_from_text(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        self.logger.warning(f"Could not parse JSON from LLM response, using fallback")
        return {
            "allowed": False,
